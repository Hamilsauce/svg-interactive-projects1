...
const plotPoints = (svg, radius, numberOfPoints) => {
...
export const drawShape = (svg, radius, pointCount = 6) => {
	const points = plotPoints(svg, radius, pointCount)
	const g = document.createElementNS(SVG_NS, 'g');
	...
	const radBCR = poly.getBoundingClientRect()
	const radBBox = poly.getBBox()
	const radScreenCTM = poly.getScreenCTM()
	const radCTM = poly.getCTM()
	const { width, height } = radBBox
	
	g.setAttribute('transform', `translate(-${width/2},-${height/2}) rotate(20, 0, 0) scale(1.2)`)
...
const drawRect = (bb) => {
	...
	const hexbb = document.querySelector('.poly').getBoundingClientRect()
...
const manyAngle1 = drawShape(26, 9);
